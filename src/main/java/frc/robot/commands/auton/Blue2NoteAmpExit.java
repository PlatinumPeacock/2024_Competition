...
public class Blue2NoteAmpExit extends SequentialCommandGroup {
  /** Creates a new Blue2NoteAmpExit. */
  public Blue2NoteAmpExit(Feeder f, Intake i, Shooter s, CommandSwerveDrivetrain dt, SwerveRequest.FieldCentric d) {
    ...
    addCommands(new DriveOnly(dt, d, 1, 0.15, 0.1, -0.5));
    addCommands(new DriveOnly(dt, d, 0.05, 0, 0, 0)); //set wheels to zero to drive straight
    addCommands(new DriveOnly(dt, d, 2, 0.6, -0.05, 0)); //leave the wing
  }
}
